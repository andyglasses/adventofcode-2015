function run(){
  var lines = getData().trim().split('\n').map(function(x) { return x.trim(); });
  var lights = [];
  for(var x = 0; x < 1000; x++)
    lights[x] = new Array(1000).fill(false);
  lines.forEach(l =>
  {
    var items = l.split(' ').map(function(x) { return x.trim(); });
    var step = items.shift();
    if(step == "turn")
    {
      step = items.shift();
    }
    ...
    switch(step){
      ...
      case "off": func = off;
      break;
      case "toggle": func = toggle;
    }
    var fromCo = items.shift().split(',').map(function(x) { return parseInt(x.trim()); });
    var through = items.shift();
    var toCo = items.shift().split(',').map(function(x) { return parseInt(x.trim()); });
    if(items.length > 0){
      console.log("items left")
      return;
    }
    if(through !== "through"){
      console.log("through no through")
      return;
    }
    if(fromCo[0] > toCo[0] || fromCo[1] > toCo[1]){
      console.log("complex-coords");
      return;
    }
    for(var x = fromCo[0]; x <= toCo[0]; x++)
      for(var y = fromCo[1]; y <= toCo[1]; y++){
...
function getData(){
  return `turn on 887,9 through 959,629
